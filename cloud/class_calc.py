...
設計= 設計書。対応= **高知**(令和8年度 番組編成要領 4./5./6.(4))・
...
#           九州産重賞とステップ 20%・2歳新馬戦= 令和2・3年度 50%/令和4年度 30%/令和5年度 20%/令和6年度以降 50%(各年度の要領)・他場遠征 10%・協賛金競走 20%(⚠名前で判別できず=100%)。
...
def saga_rate_resident(run, race):
    ...
    if kind == "重賞" or "Ｊｐｎ" in name or "Jpn" in name:
        ...
        if re.search(r"[２３]歳", name):
            return 0.30, "2歳3歳重賞"
        return 0.50, "重賞"
    if "九州産" in name and (kind in ("準重賞", "特別") or "チャレンジ" in name):
        # (リ) 九州産重賞のステップ競走 20%。「たんぽぽ賞チャレンジカップ２歳九州産」は台帳の種別が普通(§288)
        return 0.20, "九州産ステップ"
    if "九州馬主協会" in name or "九州軽種馬協会" in name:
        # (ヘ) 九州馬主協会及び九州軽種馬協会の協賛金拠出競走 20%(名前に協会名があるものだけ。他の協賛は名前で判別できない)
        return 0.20, "協賛金"
    if "新馬" in name and d:
        # 2歳新馬戦の率は、その年度の要領どおり(2026-09-26 原文で確認・BACKTEST-demotion-saga-20260926.md):
        #   R4 要領(ホ)= 令和2・3年度 50%・令和4年度以降 30% / R5 要領= 令和5年度以降 20% / R6〜R8 要領= 令和6年度以降 50%
        fy = _fy(d)
        if fy >= 2024:
            return 0.50, "2歳新馬"
        if fy == 2023:
...
def saga_rate_entry(run, age_entry):
    ...
    if not jra and ("認定" in str(run.get("name") or "") or "Ｊ認" in str(run.get("name") or "")):
        return 0.50, "他場認定"                     # (ニ) 他場におけるＪＲＡ認定競走 50%(§288・2・3歳の転入でも)
