...
AUCTION_NEG_TAIL = re.compile(
    ...
    r"|"
    # §205 動詞の活用を挟む打ち消し=「歩様が乱れたことはなく」「跛行するようなことはなく」「歩様の乱れが出ることはなく」
    #   「歩様異常が出たことはありません」。⛔前の 12 字に「。」「、」を跨がない(別の節の打ち消しを拾わない)
    r"[^。、]{0,12}?(?:た|る|する|するような|が出た|が出る|になった|になる)?こと(?:は|も|が)?"
    r"(?:一度も|特に|これまで|今まで)?(?:なく|ない|ありません|ございません|なかった)"
    r")")
