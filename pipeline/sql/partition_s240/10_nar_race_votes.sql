...
begin;
set local statement_timeout = '30min';
delete from public.nar_race_votes where race_date >= '2022-11-01' and race_date < '2024-01-01';
commit;

--   2024
begin;
set local statement_timeout = '30min';
delete from public.nar_race_votes where race_date >= '2024-01-01' and race_date < '2025-01-01';
commit;

--   2025
begin;
set local statement_timeout = '30min';
delete from public.nar_race_votes where race_date >= '2025-01-01' and race_date < '2026-01-01';
commit;

--   2026(今年)
begin;
set local statement_timeout = '30min';
delete from public.nar_race_votes where race_date >= '2026-01-01' and race_date < '2027-01-01';
commit;

--   2027 以降
begin;
set local statement_timeout = '30min';
delete from public.nar_race_votes where race_date >= '2027-01-01';
commit;

-- 3-2 delete で空いた場所を返す(⛔vacuum はトランザクションの中では流せない。
--   ⛔vacuum full は使わない= 表と同じ大きさの場所を新しく要求し、排他ロックで画面が止まる)
vacuum (analyze) public.nar_race_votes;
...
begin;
set local statement_timeout = '30min';
alter table public.nar_race_votes validate constraint nar_race_votes_archive_ck;
commit;
...
set local lock_timeout = '10s';
set local statement_timeout = '30min';
alter table public.nar_race_votes_p
  attach partition public.nar_race_votes for values from (minvalue) to ('2022-11-01');
commit;

-- 3-6 確認= 区画が 6 つ・親の索引がすべて indisvalid
select relid::regclass as part, level from pg_partition_tree('public.nar_race_votes_p');
select i.indexrelid::regclass as idx, i.indisvalid
  from pg_index i where i.indrelid = 'public.nar_race_votes_p'::regclass;


-- =====================================================================
-- §4 名前の入れ替え(⛔ここだけ短い排他。区画の名前はそのまま)
--   旧表 nar_race_votes → nar_race_votes_archive_part(親の下の archive 区画のまま)
--   親   nar_race_votes_p → nar_race_votes(画面・便・PostgREST の URL は変わらない)
