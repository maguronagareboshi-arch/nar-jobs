-- ⛔9/23 から odds_archive.py に置き換え(監査 #28)= nar-refresh.yml からは呼ばない。書き出し前に間引くと消える。
--   60 日超は cloud/odds_archive.py が Storage odds-archive へ書き出し→照合→消す。このファイルは記録として残す。
